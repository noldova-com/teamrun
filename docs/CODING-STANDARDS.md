...
- Directories and source filenames use lowercase kebab-case derived from the primary concept: `snapshot-tracker.ts` for `SnapshotTracker`.
- Classes, enums, and other named runtime types use descriptive PascalCase; interfaces use `I` plus PascalCase (`IProviderAdapter`); decorators use PascalCase.
...
- Files tested through foundation Testing mirror the source filename plus `.test`; test classes add `Tests` to the production type (`SnapshotTrackerTests`). Angular and Playwright conventions follow section 13.
...
- Enums use explicitly assigned string values that are camelCase renderings of their member names (`InvalidParams = "invalidParams"`), except enums of wire names, whose values are the names on the wire (`MethodName.ProviderList = "provider/list"`); numeric values only when numeric meaning is required (bit flags, binary formats), never mixed, never implicit ordering. The enum owns its literals, not `Resources`.
