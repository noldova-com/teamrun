...
@TestClass
export class MemberRemoveCommandTests {
  @TestMethod
  public async exercisesTheRuntimeContractAndUsage(): Promise<void> {
    ...
    Assert.areEqual(0, await host.run("member-remove", conversation.id, teammate.id));
    ...
    Assert.areEqual(0, await host.run("member-remove", conversation.id, teammate.id));
    Assert.areEqual(2, await host.run("member-remove", conversation.id));
  }
}
