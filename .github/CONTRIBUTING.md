...
The release workflow declares its signed platforms once, as `RELEASE_SIGNED_PLATFORMS` in its validation job; it lists none yet. Packaging jobs for a signed platform run in the protected `release` environment, which holds the signing secrets and allows only `main` and `v*` tags. Unsigned jobs and pull-request checks never reach it. On macOS the job signs with the Developer ID Application certificate and the hardened runtime, notarizes with an App Store Connect API key, staples the ticket, and checks the app with `codesign`, `spctl` and `stapler`. The publisher rejects any package whose signing differs from the declared platforms. The account holder creates the environment and its secrets: `MAC_CERTIFICATE` (the base64-encoded `.p12` with the certificate and its private key), `MAC_CERTIFICATE_PASSWORD`, `APPLE_API_KEY_P8` (the API key file contents), `APPLE_API_KEY_ID` and `APPLE_API_ISSUER`. Windows signing is not configured yet.
